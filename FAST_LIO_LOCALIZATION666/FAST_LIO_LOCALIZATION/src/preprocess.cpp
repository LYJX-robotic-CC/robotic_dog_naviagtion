...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    int timestamp_datatype = sensor_msgs::PointField::FLOAT64;
    ...
    auto normalize_time_offset_ms = [&](double raw_time) -> float {
      if (!std::isfinite(raw_time) || raw_time <= 0.0) return 0.0f;
      const double header_time = msg->header.stamp.toSec();
      if (header_time > 0.0 && raw_time > header_time - 1.0 && raw_time < header_time + 1.0)
      {
        return static_cast<float>(std::max(0.0, (raw_time - header_time) * 1000.0));
      }
      // Some Robosense drivers publish absolute per-point timestamps while the
      // ROS header uses a different clock base. Do not feed that huge offset to
      // FAST-LIO; yaw-based timing below is safer than a bogus scan duration.
      if (raw_time > 10.0) return 0.0f;
      return static_cast<float>(raw_time * 1000.0);
    };

    std::vector<PointType> points;
    std::vector<int> rings;
    std::vector<float> offset_ms;
    points.reserve(plsize);
    rings.reserve(plsize);
    offset_ms.reserve(plsize);
    ...
    if (has_time)
    {
      ...
      for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z, ++iter_intensity, ++iter_ring, ++iter_time)
      {
        ...
        if (!std::isfinite(range) || range <= blind || range > max_scan_range * max_scan_range) continue;
        points.push_back(added_pt);
        rings.push_back(static_cast<int>(*iter_ring));
        offset_ms.push_back(normalize_time_offset_ms(static_cast<double>(*iter_time)));
      }
    }
    else if (has_timestamp && timestamp_datatype == sensor_msgs::PointField::FLOAT64)
    {
      ...
      for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z, ++iter_intensity, ++iter_ring, ++iter_timestamp)
      {
        ...
        points.push_back(added_pt);
        rings.push_back(static_cast<int>(*iter_ring));
        offset_ms.push_back(normalize_time_offset_ms(*iter_timestamp));
      }
    }
    else if (has_timestamp)
    {
      ...
      for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z, ++iter_intensity, ++iter_ring, ++iter_timestamp)
      {
        ...
        points.push_back(added_pt);
        rings.push_back(static_cast<int>(*iter_ring));
        offset_ms.push_back(normalize_time_offset_ms(static_cast<double>(*iter_timestamp)));
      }
    }
    else
    {
    ...
    if(feature_enabled)
    {
      ...
      {
        PointType added_pt = points[i];
        const int layer = rings[i];
        if (layer < 0 || layer >= N_SCANS) continue;
        added_pt.curvature = offset_ms[i];
